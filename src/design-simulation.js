...
export const defaults={variant:'base',kind:'thread',interval:300,payload:256,battery:2000,batteryV:3.7,efficiency:85,baseMw:100,supply:500,railV:3.3,signal:'good',network:true,sim:true,antenna:true,overrides:{}};
...
export function normalize(input={}){
 ...
 for(const [k,min,max] of [['interval',1,86400],['payload',1,1000000],['battery',1,100000],['batteryV',1,60],['efficiency',1,100],['baseMw',0,10000],['supply',1,10000],['railV',1,6]])s[k]=clamp(s[k],min,max,defaults[k]);
...
export function evaluate(input,kind=input.kind){
 ...
 const totalMw=radioMw+s.baseMw,batteryWh=s.battery*s.batteryV/1000*s.efficiency/100;
 const days=totalMw>0?batteryWh/(totalMw/1000)/24:null;
 return {kind,p,cycle,attach,tx,attempts,utilization,requested,delivered,backlog:requested-delivered,problems,connected,required,radioMw,totalMw,days,dataMB:delivered*(s.payload+p.overhead)*attempts*30/1e6,offeredMB:requested*s.payload*30/1e6,energyMwh:totalMw*s.interval/3600};
}
export function sequence(input){const s=normalize(input),r=evaluate(s),steps=[{label:'Read sensors',detail:`Prepare ${s.payload} bytes`,duration:0.1}];
 if(r.problems.some(p=>p.startsWith('Supply')||p.startsWith('Radio rail')))return [...steps,{label:'Power fault',detail:r.problems.join(' · '),duration:0.1,failed:true}];
